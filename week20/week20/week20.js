	...
	try {
		...
		class Person {
			constructor (name, age, city) {
				if(age < 18) throw new Error("К сожалению, Вы еще не достигли совершеннолетия");
				this.name = name;
				this.age = age;
				this.city = city;
			}
		}
		const person = new Person("Mark", 15, "Berlin");
		resultThree.textContent = 'Функция вызвана без ошибок';
	} catch (error) {
...
function makeFive() { //// ???? правильно ли
	let inputData = -1;
	if (inputData < 0) 
		//Ваш код
...
function makeSix() {
	...
	return inputData == 0 ? 1 : inputData * (inputData - 1);
		...
		console.log ('Ошибка была перехвачена:', error);
	}
}
...
function makeTen() {
	...
	if (!emailPattern.test(email)) {
		...
		resultTen.textContent = 'Ошибка : '+ error;
...
function makeEleven() {
	...
	//Ваш код
		...
		throw new Error ('Телефонный номер должен содержать не менее 10 цифр');
	} else{
		...
		resultEleven.textContent = 'Ошибка : '+ error;
		document.getElementById('result11').textContent = 'Ошибка : ' + error;
		console.log('Ошибка : ' + error);
	}
}
...
function makeThirteen() {
	...
	if (age < 18) {
		...
		resultThirteen.textContent = 'Ошибка : '+ error;
		//document.getElementById('result13').textContent = 'Ошибка : ' + error;
		console.log('Ошибка : ' + error);
	}
}	
...
function makeFourteen() {
	...
	if (discount < 0 || discount > 50) {
		//Ваш код
		throw Error ("Некорректное значение скидки")
	} else {
		resultFourteen.textContent = 'Размер скидки составляет : ' + discount;
		console.log('Размер скидки составляет : ' + discount);
	}	
}
catch (error){
	resultFourteen.textContent = 'Ошибка : '+ error;
	//document.getElementById('result14').textContent = 'Ошибка : ' + error;
	console.log('Ошибка : ' + error);
}
}
...
function makeFifteen(a, func) {
	...
	try {
		...
		//Ваш код
		resultFifteen.textContent = 'Ошибка : '+ error;
		console.log('Ошибка : ' + error);
	}
}
...
function makeSixteen(n, func) {
	...
	func(n)
		...
		resultSixteen.textContent = 'Такой функции не существует'
	}
}
...
function makeSeventeen(str, func) {
	...
	try {
		func(str)
			...
			resultSeventeen.textContent = 'Такой функции не существует'
		}
}
...
function makeEighteen() {
	...
	//Блок finally
		...
		console.log("finnaly"); //Вывод ошибки в консоль
		resultEighteen.textContent = 'finnaly'; //Вывод 'finnaly' 	
	}
}
...
function makeNineteen() {
	...
	const finallyNinetenn = document.getElementById('result19a');
	//Блок try 
	try {
	const arr = [1, 2, 3];
	...
	//Блок finally
		...
		console.log("finally19"); //Вывод ошибки в консоль
		finallyNinetenn.textContent = 'finally19'; //Вывод 'finnaly' 	
	}
}
...
function makeTwenty() {
	...
	try {
		const functionResult = calculateValue();
		resultTwenty.textContent = functionResult;
		console.log(functionResult);	
}
catch(error) {
	resultTwenty.textContent = 'Ошибка';
	console.error(error, 'Ошибка'); //Вывод ошибки в консоль
}
finally {
	console.log("finally20"); //Вывод  в консоль
	finallyTwenty.textContent = 'finally20'; //Вывод 'finnaly' 	
}
}
...
function makeTwentyOne() {
	...
	try {
		const functionResult = fetchData();
		resultTwentyOne = functionResult;
}
catch(error) {
	...
	console.log("finally21"); //Вывод ошибки в консоль
	finallyTwentyOne.textContent = 'finally21'; //Вывод 'finnaly' 	
}
}
...
function makeTwentyFour() {
	...
	//Блок catch (error)
		...
		resultTwentyFour.textContent = 'Ошибка :' + error;
		console.log('Ошибка :' + error);
	}
}
...
function makeTwentyFive() {
	...
	//Блок catch (error)
		...
		resultTwentyFive.textContent = 'Ошибка :' + error;
		console.log('Ошибка :' + error);
	}
}
...
function makeTwentySix() {
	...
	setTimeout(() => {
        ...
        resultTwentySix.textContent = "Функция выполнена корректно"
	//Блок catch (error)	
		} catch (error) {
